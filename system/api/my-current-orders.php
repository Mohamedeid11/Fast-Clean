...
if (isset($_GET['client_id']) && $_GET['lang'] != '') {
    ...
    $result = mysql_query("SELECT * FROM `orders` WHERE `client_id`='$client_id' and ((`order_follow`!=3 and `order_follow`!=0 ) or `order_status`=0) ORDER BY `order_id` DESC") or die(mysql_error());
...
//                $results["spicy_type"] = $row_select["spicy_type"];
                ...
                if ($lang == "ar") {
                    ...
                    $results["vehicle_name"] = get_vehicle_name_ar_from_id($washer_id);
                    $results["subscription_name"] = get_subscription_name_ar_from_id($washer_id);
                } else {
                    $results["washer_name"] = get_washer_name_en_from_id($washer_id);
                    $results["vehicle_name"] = get_vehicle_name_en_from_id($washer_id);
                    $results["subscription_name"] = get_subscription_name_en_from_id($washer_id);
                }
...
//                $total_price_without_charge = $net_price - $charge_cost;
                ...
                array_push($washer_arr_values, $results);
            }
...
//                                "charge_cost" => number_format((float) ($charge_cost), 3, '.', ''),
//                                "total_price_without_charge" => number_format((float) ($total_price_without_charge), 3, '.', ''),
//                                "net_price" => number_format((float) ($net_price), 3, '.', ''), "items" => $washer_arr_values
            );
