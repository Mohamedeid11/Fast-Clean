...
function get_washer($washer_id , $lang ,$client_id) {

    $data = array();
    if ($client_id > 0) {
        $query = mysql_query(" UPDATE comments SET viewed = viewed + 1 WHERE washer_id = '{$washer_id}' ");
    }
    $result = mysql_query("SELECT * FROM `washers` WHERE `washer_id`='$washer_id' and `display`=1 order by washer_id ASC") or die(mysql_error());
