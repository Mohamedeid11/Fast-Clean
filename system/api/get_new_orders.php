...
mysql_query("SET SESSION collation_connection = 'utf8_unicode_ci'");
        ...
        $run_query = $run_query . " FROM     orders where IF(IFNULL(mobile_type, '') = '', 'ios', mobile_type) in ('ios','android') ";
...
$run_query = "SELECT  orders.client_id,orders.deliver_id,orders.order_follow,orders.order_id,cast((orders.total_price-orders.vat)  as decimal(18,3))  AS Amount,
        cast(orders.net_price as decimal(18,3))  AS Net_Amount,cast(orders.charge_cost as decimal(18,3))  AS Charge,cast(orders.vat as decimal(18,4))  AS VAT,
        cast((orders.net_price-orders.vat-orders.charge_cost)/((100-discount_percentage)/100) as decimal(18,3))-cast(orders.net_price-orders.vat-orders.charge_cost as decimal(18,3))
        AS discount, orders.date AS OrderDate, (select user_addresses_session.name from user_addresses_session where user_addresses_session.id=orders.client_address_id ) AS ClientName,     
        (select user_addresses_session.phone from user_addresses_session where user_addresses_session.id=orders.client_address_id )  AS Phone,     orders.payment AS Payment,(SELECT   region_name_en  FROM            
        regions         WHERE             regions.region_id = (select user_addresses_session.region from user_addresses_session where user_addresses_session.id=orders.client_address_id))
        AS Area,(select block from user_addresses_session where user_addresses_session.id=orders.client_address_id) AS Block,(select road from user_addresses_session where 
        user_addresses_session.id=orders.client_address_id) AS Road,(select building from user_addresses_session where user_addresses_session.id=orders.client_address_id)
        AS Building,(select flat_number from user_addresses_session where user_addresses_session.id=orders.client_address_id) AS Flat,(select note from user_addresses_session 
        where user_addresses_session.id=orders.client_address_id) AS Note,order_status as status,  (select name_en from delivered_way where id=orders.deliver_id)   
        as OrderMode,mobile_type  " ;
        ...
        $run_query = $run_query . " FROM     orders where mobile_type in ('Web') ";
