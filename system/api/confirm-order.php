...
if (isset($postdata) && !empty($postdata)) {
    ...
    foreach ($carts_array as $one_cart) {
        ...
        $check_category_exist = check_category_exist($washer_id);
        if ($check_category_exist == 0) {
            $not_exist = 1;
        }
        else {
            $check_washer_exist = check_washer_exist($washer_id);
            if ($check_washer_exist == 0) {
                $not_exist = 1;
            }
        }
    }
...
//        $total = $total_amount_after_disc + $get_charge;
        $vat_added = number_format((float) ( (($vat / 100) * $total_amount_after_disc)), 3, '.', '');
//        $net_price_after_vat = number_format((float) ($total_amount_after_disc + (($vat / 100) * $total_amount_after_disc)), 3, '.', '');
        ...
        $result = mysql_query("INSERT INTO `orders` (`washer_id`, `cart_id`, `client_id`, `client_address_id`, `subscription_id`, `vehicle_id`, `total_price`, `discount_percentage`, `discount_value`, `vat`, `vat_percentage`, `order_status`, `order_follow`, `payment`, `mobile_type`, `date`) 
                                                VALUES ('$washer_id', '$cart_id', '$client_id', '$client_address_id', '$subscription_id', '$vehicle_id', '$total_amount', '$discount_percentage', '$discount_value', '$vat_added', '$vat', '0', '0', '$payment', '$mobile_type', '" . date("Y-m-d H:i:s") . "')");
        ...
        if ($result) {
            ...
            $to = "apps@emcan-group.com";
            $subject = "Fast Clean - New Order";
            $headers = 'MIME-Version: 1.0' . "\r\n";
            $headers .= 'Content-type: text/html; charset=utf-8' . "\r\n";
            $headers .="From:  Fast Clean" . " Info@emcan-group.com\r\n" . "Reply-To:Info@emcan-group.com\r\n";
            $message = send_email($order_id, $client_id, $client_address_id);
            mail($to, $subject, $message, $headers);
