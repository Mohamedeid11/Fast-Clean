...
if (isset($_POST['change_status_on'])) {
    ...
    $query = $con->query("UPDATE `category` SET `display`=1 WHERE `id`='$change_status'");
...
if (isset($_POST['change_status_off'])) {
    ...
    $query = $con->query("UPDATE `category` SET `display`=0 WHERE `id`='$change_status'");
...
if (isset($_POST['washer_id'])) {
    ...
    $querya = $con->query("SELECT * FROM `category` WHERE `id`='$id' limit 1");

    $row_select = mysqli_fetch_array($querya);

    $query = $con->query("DELETE FROM `category` WHERE `id`='$id'");
