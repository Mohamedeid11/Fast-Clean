...
if (isset($_POST['change_status_on'])) {
    ...
    $query = $con->query("UPDATE `vehicle_type` SET `display`=1 WHERE `id`='$change_status'");
...
if (isset($_POST['change_status_off'])) {
    ...
    $query = $con->query("UPDATE `vehicle_type` SET `display`=0 WHERE `id`='$change_status'");
...
if (isset($_POST['vehicle_id'])) {
    ...
    $querya = $con->query("SELECT * FROM `vehicle_type` WHERE `id`='$id' limit 1");

    $row_select = mysqli_fetch_array($querya);

    $query = $con->query("DELETE FROM `vehicle_type` WHERE `id`='$id'");
