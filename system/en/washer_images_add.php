    ...
    if (isset($_POST['submit'])) {
        ...
        if (!empty($errors)) {
            ...
            for($i=0 ;$i<$imageCount ;$i++) {
                ...
                $con->query("INSERT INTO `washer_images` ( `washer_id`, `image`) VALUES ( '$washer_id', '$image')");

                $id = mysqli_insert_id($con);
                if (!file_exists("../api/uploads/Washer_images/" . $id)) {
                    mkdir("../api/uploads/Washer_images/" . $id, 0777, true);
                }
                $image_path = "../api/uploads/Washer_images/" . $id . "/" . $image;
                $image_database = "{$sit_url}/api/uploads/Washer_images/" . $id . "/" . $image;

                if (move_uploaded_file($image_tmp, $image_path)) {
                    $update = $con->query("UPDATE `washer_images` SET  `image`='$image_database' WHERE `id`='$id'");
                }
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            ...
                            <ol class="breadcrumb">
                                ...
                                <li class="active"><?=lang('add_new_washer_image')?></li>
                            </ol>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="card-box">
                                <h4 class="m-t-0 header-title"><b> <?=lang('add_new_washer_image')?></b></h4>
