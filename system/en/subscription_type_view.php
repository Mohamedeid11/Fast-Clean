                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            ...
                            <ol class="breadcrumb">
                                ...
                                <li class="active"><?= lang('subscription')?>   </li>
                            </ol>
                        </div>
                    </div>
                    <h4>
                    ...
                    <div class="panel">
                        <div class="panel-body">
                            <table class="table table-striped" id="datatable-editable">
                                ...
                                $query = $con->query("SELECT * FROM `subscription_type` ORDER BY `id` ASC");
                                $x = 1;
                                while ($row = mysqli_fetch_assoc($query)) {
                                    $subscription_id = $row['id'];
                                    $subscription_name_en = $row['subscription_name_en'];
                                    $subscription_name_ar = $row['subscription_name_ar'];
                                    $display = $row['display'];
                                    $date = $row['date'];
                                    ?>
                                    <tr class="gradeX<?= $subscription_id ?>">
                                        <td><?php echo $x; ?></td>
                                        ...
                                        <td>
                                            <?php if ($display == 1) { ?>
                                                <input class="change_status_off" data-id="<?=$subscription_id; ?>" type="checkbox"
                                                       checked
                                                       data-plugin="switchery" data-color="#81c868"/>
                                                <?php
                                            }else if ($display == 0) {
                                                ?>
                                                <input class="change_status_on" data-id="<?=$subscription_id; ?>" type="checkbox"
                                                       data-plugin="switchery" data-color="#81c868"/>
                                                <?php
                                            }
                                            ?>
                                        ...
                                        <td><?= $date; ?></td>
                                        <td class="actions">
                                            <a href="subscription_type_edit.php?subscriptionID=<?= $subscription_id; ?>" class="on-default"><i class="fa fa-pencil"></i></a>
                                        </td>
                                        <td class="actions">
                                            <a href="<?= $subscription_id; ?>" class="on-default remove-row"><i class="fa fa-trash-o"></i></a>
                                        </td>
                                    </tr>
                                    <?php
                                    $x++;
...
<script type="text/javascript">
    $(document).ready(function () {
        $('body').on('click', '.on-default', function () {
            var subscription_id = $(this).attr('href');
            // alert(category);
            $("#dialogConfirm").click(function () {
                var dataString = 'subscription_id=' + subscription_id;
                $.ajax({
                    type: "POST",
                    url: "functions/subscription_function.php",
                    data: dataString,
                    dataType: 'text',
                    cache: false,
